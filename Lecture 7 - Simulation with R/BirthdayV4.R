#This script is the simulation model to handle Birthday problem V4
...
GetExpectedNumWinners <- function(error.tolerance, num.cust){
    ...
    simulate(num.iterations,num.cust)
}
...
PrizeforGame<-function(num.cust, error.tolerance) {
    ...
    cat("Number of customers: ",num.cust,"\n")
    cat("Max prize to pay per customer: ", max.prize,"\n")
    return(max.prize)
}

PlotMaxPrize <- function(min.num.cust, max.num.cust, error.tolerance){
    library(ggplot2)
    num.custs <- min.num.cust : max.num.cust
    probs <- sapply(num.custs,PrizeforGame,error.tolerance)
    
    df <- data.frame(NumOfCusts = num.custs, Probabilities = probs)
    ggplot(df, aes(x=NumOfCusts, y=Probabilities)) + geom_line(color="blue") + labs(x="Number of Customers", y="Max prize to pay") + geom_hline(yintercept=150,lty=2,color="red")
}